...
**Kanpeki** is a collection of beautifully designed, accessible, and composable components for your React apps. Built on top of [React Aria Components](https://react-spectrum.adobe.com/react-aria/) and styled with [Tailwind CSS](https://tailwindcss.com/), it's designed for you to copy, paste, and own.
...
- **New features:**
  - `compose` - Shallow merge multiple cva components
  - `defineConfig` - Extend cva globally
  - Cleaner API with single parameter (base styles via `base` property)
...
Instead of `tw-animate-css`, we use [`tailwindcss-motion`](https://romboui.com/tailwindcss-motion/) for animations. It provides:
